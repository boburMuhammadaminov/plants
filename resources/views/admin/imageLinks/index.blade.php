        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Foydali link(rasm) qo'shish</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- form start -->
                    <form method="POST" action="{{ route('admin.imageLinks.store') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body">
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-10 mb-3">
                            <h3 class="card-title">Foydali linklar(rasm)</h3>
                        </div>
                        <div class="col-md-2">
                ...
                <div class="card-body p-0 table-responsive">
                    <table class="table table-sm">
                        ...
                        @foreach($imageLinks as $imageLink)
                            <tr>
                                <td>{{ $loop->index+1 }}</td>
                                <td>{{ Str::limit($imageLink->title_uz, 60) }}</td>
                                <td>{{ $imageLink->link }}</td>
                                <td>{!! $imageLink->getImage('width:150px') !!}</td>
                                <td>
                                    <div class="btn-group">
                                        <form action="{{route('admin.activation', ['id'=>$imageLink->id])}}" method="POST">
                                            <input type="hidden" name="type" value="imageLink">
                                            @csrf
                                            {!! $imageLink->getStatus() !!}
                                        </form>
                                    </div>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ route('admin.imageLinks.show', $imageLink->id) }}" type="button"
                                           class="btn btn-primary btn-flat">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.imageLinks.edit', $imageLink->id) }}" type="button"
                                           class="btn btn-success btn-flat">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{route('admin.imageLinks.destroy',$imageLink->id)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
