<?php

return [
    'site map' => 'Карта сайта',
    'mobile version' => 'Мобильная версия',
    'visually impaired' => 'Для слабовидящих',
    'read aloud' => 'Читай вслух',
    'search' => 'Поиск',
    'our address' => 'Наш адрес',
    'email address' => 'Эл. адрес',
    'helpline' => 'Телефон доверия',
    'rights reserved' => 'Все права защищены',
    'site creator' => 'Создатель сайта',
    'home' => 'Главная',
    'news' => 'Новости',
    'gallery' => 'Галерея',
    'f-size' => 'Размер шрифта',
    'color' => 'Цвет сайта',
    'see-all' => 'Увидеть все',
    'contact' => 'Контакт', 
    'name' => 'Ваше имя', 
    'phone' => 'Ваш телефон', 
    'email' => 'Ваш адрес электронной почты', 
    'message' => 'Твое сообщение', 
    'send' => 'Отправить', 
    'information' => 'Информация', 
    'autobiography' => 'Автобиография', 
    'charges' => 'Обязанности', 
    'staffPhone' => 'Телефон', 
    'staffEmail' => 'Эл. почта', 
    'staffReception' => 'Дни приема',  
    'usefullLinks' => 'Полезные ссылки', 
    'ourStaff' => 'Наша команда', 
    'video' => 'Видео', 
    'successfullySend' => 'Успешно отправлено!', 
    'goodJob' => 'Молодец!', 
    'footer1' => 'При использовании материалов опубликованных на данном сайте, ссылка на www.karantinhimoya.uz обязательна.', 
    'footer2' => 'Все материалы на сайте доступны по следующим лицензиям: ', 




];